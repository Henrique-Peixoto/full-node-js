...
INSERT INTO posts (user_id, title, status, views)
SELECT id, 'Backend APIs with PostgreQSL', 'published', 180
FROM users
WHERE name = 'Rahul';
...
FROM posts
WHERE title = 'Backend APIs with PostgreQSL';
...
SELECT p.id, t.id
FROM posts p, tags t
WHERE p.title = 'Backend APIs with PostgreQSL' AND t.name = 'backend';
